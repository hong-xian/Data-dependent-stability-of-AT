...
def craft_poison_random(args, train_loader, test_loader, model):
    # Generate random perturbations for each class
    poisons = torch.zeros(args.num_classes, *args.data_shape).cuda(non_blocking=True)
    step = STEPS[args.constraint](None, args.poison_eps, None)
    poisons = step.random_perturb(poisons)
    # fixed random classwise noise for train and test
    poisoned_data_train = poison_random(args, train_loader, model, poisons)
    poisoned_data_test = poison_random(args, test_loader, model, poisons)
    return poisoned_data_train, poisoned_data_test
...
def poison_random(args, loader, model, poisons):
    ...
    for i, (inp, target) in iterator:
        ...
        desc = ('[{} {}] | Loss {:.4f} | Accuracy {:.3f} ||'
                .format(args.poison_type, args.poison_eps, loss_logger.avg, acc_logger.avg))
        iterator.set_description(desc)
...
def main(args):
    ...
    model.eval()
    if args.poison_type == "Adv" or args.poison_type == "Hyp":
        ...
        poison_data_train, poison_data_test = craft_poison_random(args, train_loader, test_loader, model)
        torch.save(poison_data_train, args.poison_path)
        torch.save(poison_data_test, args.poison_test_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default="CIFAR10",
                        ...
                        help='choose the dataset to generate poison')
