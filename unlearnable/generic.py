...
import unlearnable.data as data
import models
...
def get_dataset(dataset, root='./data', train=True):
    ...
    if dataset == 'CIFAR10':
        ...
        if train:
            target_set = datasets.SVHN(root=root, split="train", transform=transform, download=True)
            target_set.data = np.transpose(target_set.data, (0, 2, 3, 1))
            x, y = target_set.data, target_set.labels
        else:
            target_set = datasets.SVHN(root=root, split="test", transform=transform, download=True)
            target_set.data = np.transpose(target_set.data, (0, 2, 3, 1))
            x, y = target_set.data, target_set.labels
    else:
...
def get_concat_dataset(dataset, root='./data'):
    ...
    if dataset == 'CIFAR10':
        ...
        train_set = datasets.SVHN(root=root, split="train", transform=transform, download=True)
        test_set = datasets.SVHN(root=root, split="test", transform=transform, download=True)
        x_1, y_1 = train_set.data, train_set.labels
        x_2, y_2 = test_set.data, test_set.labels
        x = np.concatenate([x_1, x_2])
...
def get_arch(arch, dataset):
    if dataset == "CIFAR10":
        ...
        if arch == 'ResNet18':
            ...
            model = models.VGG('VGG16', num_classes=10)
        elif arch == 'WRN28-10':
            model = models.WideResNet(depth=28, num_classes=10, widen_factor=10)
    else:
        raise NotImplementedError('dataset {} is not supported'.format(dataset))
...
def get_optim(optim, params, lr=0.1, weight_decay=5e-4, momentum=0.9):
    if optim == 'sgd':
